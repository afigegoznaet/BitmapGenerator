...
struct BMPFileHeader {
	uint16_t file_type{0x4D42}; // File type always BM which is 0x4D42 (stored
								// as hex uint16_t in little endian)
...
struct BMP {
	...
	void draw_rectangle(uint32_t x0, uint32_t y0, uint32_t w, uint32_t h,
		...
		if (x0 + w > (uint32_t)bmp_info_header.width
			|| y0 + h > (uint32_t)bmp_info_header.height) {
			throw std::runtime_error(
				"The rectangle does not fit in the image!");
		}

		fill_region(x0, y0, w, line_w, B, G, R, A); // top line
		fill_region(x0, (y0 + h - line_w), w, line_w, B, G, R,
					A); // bottom line
		fill_region((x0 + w - line_w), (y0 + line_w), line_w,
					(h - (2 * line_w)), B, G, R, A); // right line
...
private:
	...
	void check_color_header(BMPColorHeader &bmp_color_header) {
		...
		if (expected_color_header.red_mask != bmp_color_header.red_mask
			...
			|| expected_color_header.alpha_mask
				   != bmp_color_header.alpha_mask) {
			throw std::runtime_error(
